"""In this file, players can choose one of two paths to take in a game that lets you hang out with friends or your significant other."""
...
def greet() -> None:
    """Function that greets the player once they start the game."""
    global player
    ...
    print("Points in each scenario will be rewarded based on if the answers you give to the given question is the best answer possible.")
    print("If you do not answer with 'Yes' or 'No' exactly, you will be awarded points as if you answered with the worse choice. ")


def friends() -> None:
    """Function that creates an envrionment where you can hang out with friends."""
    global points 
    print(f"*You will now be put in a group of your friends, and will earn points based on how good of a friend you are {ROFL}.*" "\n")
    pizza_place: str = input(f"Yo {player}, I really want pizza right now. Do you want to get Dominoes?" "\n")
    if pizza_place == "Yes":
        points += GOOD_CHOICE
    else:
        print("Ok fine we will get Papa Johns.")
        points += BAD_CHOICE
    miles: int = random.randint(1, 50)
    driving: str = input(f"I only have {miles} miles in my gas tank, could you drive over there?" "\n")
    if driving == "Yes":
        points += GOOD_CHOICE
        print("Thank you.")
    else:
        points += BAD_CHOICE
        print("Wowwww ok, you are defintely reimbursing my gas then." "\n")
    cheese_pizza: str = input("Remember I am a vegetarian, so do you want to split a large cheese pizza to save money?" "\n")
    if cheese_pizza == "Yes":
        points += GOOD_CHOICE
    else:
        points += BAD_CHOICE
    print("*Both of you get in the car*" "\n")
...
def lover(points_in_lover: int) -> int:
    """Function that allows you to spend time with your significant other."""
    print("*answer perfectly or else this segment might abruptly end*")
    ...
    if cheating == "Yes":
        ...
        points_in_lover += GOOD_CHOICE
    date_idea: str = input("Do you have a date planned for us tonight? I'm so excited." "\n")
    if date_idea == "Yes":
...
def main() -> None:
    """Main function that contains all of the functions above."""
    global points
    global player
    points = 0
    greet()
